...
<script type="text/javascript"
	...
	<!--------------------------- 引入三级联动js文件  ------------------------------------>
<script type="text/javascript" src="<%=basePath%>js/position.js"
 ...
 var path="<%=basePath%>";
	...
	function checkquname() {
		...
		if (area_id == null || area_id == "") {
			$("#quyunamemsg").html("<font color='red'>所属区域不能为空</font>");
			return false;
		} else {
...
<form action="<%=basePath%>positionControl/updatePosition.action"   method="post"  id="form1">
	...
	<table width="100%" border="0" align="left" cellspacing="0"
		...
		<tr>
			...
			<td width="100" class="inputText"><select id="area_id"
				name="area_id" class="{required:true}" style="color: red">
					...
					<c:forEach items="${requestScope.quyuList}" var="quyu">
						<c:if test="${quyu.area_id == position.area_id}">
			...
			<div id="quyunamemsg" style="color: red"></div></td>
		...
		<tr>
			...
			<select id="department_id"    name="department_id" class="{required:true}" style="color: red">
					<c:forEach items="${requestScope.department }" var="de">
                           <c:if test="${de.department_id == position.department_id}">
		...
		<tr>
			...
			<td width="100" class="inputText"><input type="text"
				id="position_name" name="position_name" class="{required:true}"
				maxlength="16" onblur="checkponame()"
				value="${requestScope.position.position_name}" />
	...
	<input type="hidden" id="position_id" name="position_id" value="${requestScope.position.position_id}" />
</form>
</body>
</html>